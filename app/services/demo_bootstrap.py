...
from app.services.zabbix_adapter import build_demo_zabbix_alarm_payloads
...
async def ensure_demo_users(db: AsyncSession) -> tuple[User, User, User]:
    ...
    demo_user.connection_address = "B2C demo: JK-1, building 1, apt 42"
    ...
    billing_user = await _ensure_user(
        db,
        billing_id="STAFF-DEMO-BILLING",
        phone="+79005550066",
        email="billing@operator.local",
        first_name="Billing",
        last_name="Specialist",
        role=UserRole.BILLING,
        password="BillingDemo2026!",
        is_verified=True,
    )

    noc_engineer = await _ensure_user(
        db,
        billing_id="STAFF-DEMO-NOC",
        phone="+79005550088",
        email="noc@operator.local",
        first_name="NOC",
        last_name="Engineer",
        role=UserRole.NOC_ENGINEER,
        password="NocDemo2026!",
        is_verified=True,
    )

    admin = await _ensure_user(
        db,
        billing_id="STAFF-DEMO-ADMIN",
        phone="+79005550098",
    ...
    billing_user.last_login_at = datetime.utcnow() - timedelta(hours=1, minutes=28)
    noc_engineer.last_login_at = datetime.utcnow() - timedelta(minutes=33)
    admin.last_login_at = datetime.utcnow() - timedelta(minutes=25)
    super_admin.last_login_at = datetime.utcnow() - timedelta(minutes=42)
    await db.flush()
    return demo_user, operator, super_admin


async def ensure_demo_business_subscriber(db: AsyncSession) -> User:
    business_user = await _ensure_user(
        db,
        billing_id="DEMO-B2B-001",
        phone="+79005553322",
        email="business@operator.local",
        first_name="MTN",
        last_name="Business Client",
        role=UserRole.USER,
        password="DemoBusiness2026!",
        is_verified=True,
    )
    business_user.connection_address = "B2B demo: MTN Business Center, office 501"
    business_user.notification_settings = {
        "email_enabled": True,
        "sms_enabled": True,
        "push_enabled": True,
        "payment_notifications": True,
        "ticket_notifications": True,
        "account_type": "b2b",
        "connection_address": business_user.connection_address,
    }
    business_user.last_login_at = datetime.utcnow() - timedelta(hours=5, minutes=10)
    await db.flush()
...
async def ensure_demo_payments(db: AsyncSession, user: User) -> None:
    ...
    payments = [
        PaymentLog(
            ...
            external_id=f"demo-pay-{user.id}-001",
        ...
        PaymentLog(
            ...
            payment_type="topup",
            status=PaymentStatus.SUCCEEDED,
            external_id=f"demo-pay-{user.id}-002",
        ...
        PaymentLog(
            ...
            payment_type="topup",
            status=PaymentStatus.SUCCEEDED,
            external_id=f"demo-pay-{user.id}-003",
        ...
        PaymentLog(
            ...
            external_id=f"demo-pay-{user.id}-004",
...
async def ensure_demo_zabbix_alarms(db: AsyncSession) -> None:
    existing_result = await db.execute(select(ZabbixAlarm))
    existing_keys = {
        (item.alarm_type, item.source_type, item.source_name)
        for item in existing_result.scalars().all()
    }

    for payload in await build_demo_zabbix_alarm_payloads(db):
        key = (payload["alarm_type"], payload["source_type"], payload["source_name"])
        if key in existing_keys:
            continue
        db.add(ZabbixAlarm(**payload))

    await db.flush()


async def ensure_demo_noc_incidents(db: AsyncSession, operator: User, admin_user: User) -> None:
    noc_engineer = await _ensure_user(
        db,
        billing_id="STAFF-DEMO-NOC",
        phone="+79005550088",
    ...
    existing_result = await db.execute(select(NocIncident.title))
    existing_titles = {item for item in existing_result.scalars().all()}
    now = datetime.utcnow()

    if "Demo manual GPON degradation" not in existing_titles:
        db.add(
            NocIncident(
                title="Demo manual GPON degradation",
                description="Manual NOC incident for a degraded GPON segment in the demo environment.",
                severity=NocIncidentSeverity.HIGH.value,
                status=NocIncidentStatus.IN_PROGRESS.value,
                source=NocIncidentSource.MANUAL.value,
                affected_service=NocAffectedService.GPON.value,
                affected_subscribers_count=24,
                assigned_to=noc_engineer.id,
                created_by=admin_user.id,
                acknowledged_by=noc_engineer.id,
    ...
    if alarm is not None and "Demo Zabbix alarm incident" not in existing_titles:
        incident = NocIncident(
            title="Demo Zabbix alarm incident",
            description=alarm.description or "Incident created from demo Zabbix alarm.",
            severity=NocIncidentSeverity.CRITICAL.value if alarm.severity == "critical" else NocIncidentSeverity.HIGH.value,
            status=NocIncidentStatus.NEW.value,
            source=NocIncidentSource.ZABBIX.value,
            affected_service=NocAffectedService.ONT.value if alarm.source_type == "ont" else NocAffectedService.OTHER.value,
        ...
        db.add(incident)
        await db.flush()
        db.add(IncidentAlarmLink(incident_id=incident.id, zabbix_alarm_id=alarm.id))
...
async def bootstrap_demo_content(db: AsyncSession) -> None:
    ...
    demo_user, demo_operator, demo_super_admin = await ensure_demo_users(db)
    demo_b2b_user = await ensure_demo_business_subscriber(db)
    await ensure_demo_payments(db, demo_user)
    await ensure_demo_payments(db, demo_b2b_user)
    await ensure_demo_tickets(db, demo_user, demo_operator)
    await ensure_demo_tickets(db, demo_b2b_user, demo_operator)

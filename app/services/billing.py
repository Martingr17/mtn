...
class BillingService:
    ...
    def _should_use_mock_backend(self) -> bool:
        if settings.demo_mode:
            return True

        base_url = (self.base_url or "").strip().lower()
        return not base_url or "billing.operator.ru" in base_url or base_url.endswith("operator.local/api/v2")
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        if self._should_use_mock_backend():
    ...
    def _mock_response(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        default_tariff_id = "DEMO-BIZ-1500" if billing_id.startswith("DEMO-B2B") else "DEMO-300"
        active_tariff_id = MOCK_TARIFF_SELECTIONS.get(billing_id, default_tariff_id)

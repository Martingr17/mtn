...
from app.db_compat import IpAddressType, JsonType, enum_database_values
...
class ActivityLog(Base):
    ...
    action_type = Column(
        Enum(ActionType, values_callable=enum_database_values),
        nullable=True,
        comment="Action type enum",
    )
    ...
    old_value = Column(JsonType, nullable=True, comment="Previous state (for updates)")
    new_value = Column(JsonType, nullable=True, comment="New state (for updates)")
...
class AuditLog(Base):
    ...
    changes = Column(JsonType, nullable=True, comment="Changes made (before/after)")

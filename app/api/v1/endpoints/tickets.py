...
def _coerce_ticket_priority(value: object) -> TicketPriority:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Некорректный приоритет заявки") from exc
...
async def _extract_ticket_create_input(
    request: Request,
) -> tuple[str | None, str | None, TicketPriority, str | None, UploadFile | None]:
    ...
    if "application/json" in content_type:
        ...
        return (
            payload.get("subject"),
            payload.get("body"),
            _coerce_ticket_priority(payload.get("priority")),
    ...
    return (
        form.get("subject"),
        form.get("body"),
        _coerce_ticket_priority(form.get("priority")),
        form.get("category"),
        attachment if isinstance(attachment, UploadFile) else None,
    )


async def _extract_ticket_reply_input(
    request: Request,
) -> tuple[str | None, bool, UploadFile | None]:
    content_type = request.headers.get("content-type", "").lower()
    if "application/json" in content_type:
        payload = await request.json()
        if not isinstance(payload, dict):
            raise HTTPException(status_code=400, detail="Некорректное тело запроса")
        return payload.get("body"), _coerce_bool(payload.get("is_internal")), None

    form = await request.form()
    attachment = form.get("attachment")
    return (
        form.get("body"),
        _coerce_bool(form.get("is_internal")),
        attachment if isinstance(attachment, UploadFile) else None,
    )
...
async def create_ticket(
    ...
    subject, body, priority, category, attachment = await _extract_ticket_create_input(request)
...
async def reply_ticket(
    ...
    body, is_internal, attachment = await _extract_ticket_reply_input(request)
...
async def close_ticket_compat(
    ticket_id: int,
    request: Request,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Close a ticket from the user side."""
    result = await db.execute(
        select(Ticket).where(
            Ticket.id == ticket_id,
            Ticket.user_id == current_user.id,

(function () {
    ...
    function setRegisterStep(step) {
        ...
        if (elements.registerActionButton) {
            elements.registerActionButton.textContent = step === 1 ? "Отправить код на email" : "Завершить регистрацию";
        }
    }
    ...
    function resetRegisterModal() {
        ...
        setFieldError(elements.registerEmailError, elements.registerEmail, "");
        ...
        setStatus(
            ...
            "Введите телефон и email. Лицевой счёт в демо-режиме сформируется автоматически."
        );
    }
    ...
    async function handleRegisterAction() {
        ...
        if (!isValidEmail(email)) {
            setFieldError(elements.registerEmailError, elements.registerEmail, "Введите корректный email.");
            return;
        }
        ...
        try {
            if (state.register.step === 1) {
                ...
                state.register.demoCode = payload.demo_email_code || "";
                ...
                setStatus(
                    ...
                    payload.message || "Введите код из письма. Пароль можно задать сразу или пропустить.",
                    "success"
                );
                ui.toast(payload.message || "Код подтверждения отправлен на email.", "success");
            ...
            await ui.request("/api/v1/auth/register/confirm", {
                ...
                body: {
                    phone,
                    email: state.register.email || email,
                    email_code: code,
            ...
            if (state.register.step === 1) {
                if (/email/i.test(message)) {
                    setFieldError(elements.registerEmailError, elements.registerEmail, message);
                } else {

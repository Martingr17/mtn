...
async def revoke_session(
    ...
    blacklist_token(session.token, current_user.id, "session_revoked")
    if session.refresh_token:
        blacklist_token(session.refresh_token, current_user.id, "session_revoked")
...
def blacklist_token(token: str, user_id: int, reason: str):
    """Helper to blacklist token in a background task."""
    ...
    asyncio.create_task(_blacklist())
...
async def logout_other_sessions(
    ...
    for session in sessions:
        ...
        session.is_revoked = True
        revoked += 1
        blacklist_token(session.token, current_user.id, "logout_all_sessions")
        if session.refresh_token:
            blacklist_token(session.refresh_token, current_user.id, "logout_all_sessions")

...
async def get_admin_stats(
    ...
    blocked_users = await db.scalar(
        select(func.count()).select_from(User).where(
            abonent_filter,
            or_(User.is_blocked.is_(True), User.is_active.is_(False)),
        )
    ) or 0
...
async def create_admin_user(
    ...
    if payload.tariff_id:
        tariff_result = await db.execute(select(Tariff).where(Tariff.id == payload.tariff_id, Tariff.is_active.is_(True)))
...
async def list_users(
    ...
    if status_filter == "active":
        query = query.where(User.is_active.is_(True), User.is_blocked.is_(False))
    elif status_filter == "blocked":
        query = query.where(User.is_blocked.is_(True))
    elif status_filter == "inactive":
        query = query.where(User.is_active.is_(False), User.is_blocked.is_(False))

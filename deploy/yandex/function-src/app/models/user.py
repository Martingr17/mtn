...
from sqlalchemy.dialects.postgresql import INET, TIMESTAMP, JSONB
...
class User(Base):
    ...
    # Timestamps
    created_at = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, nullable=False, comment="Creation timestamp")
    updated_at = Column(TIMESTAMP(timezone=True), onupdate=datetime.utcnow, comment="Last update timestamp")
    last_login_at = Column(TIMESTAMP(timezone=True), nullable=True, comment="Last login timestamp")
    last_login_ip = Column(INET, nullable=True, comment="Last login IP address")
    
    # Preferences
    language = Column(String(2), default="ru", comment="User language preference")
    notification_settings = Column(JSONB, default={}, comment="JSON notification preferences")
...
class UserSession(Base):
    ...
    device_info = Column(JSONB, default={}, comment="Device information")
    created_at = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, nullable=False)
    expires_at = Column(TIMESTAMP(timezone=True), nullable=False, comment="Token expiration")
    last_activity_at = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
...
class TokenBlacklist(Base):
    ...
    revoked_at = Column(TIMESTAMP(timezone=True), default=datetime.utcnow, nullable=False)
    expires_at = Column(TIMESTAMP(timezone=True), nullable=False, comment="Token expiration")

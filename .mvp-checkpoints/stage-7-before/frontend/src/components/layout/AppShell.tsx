...
const SUBSCRIBER_OPS_NAV: NavItem[] = [
  { to: "/subscribers", label: "Абоненты MVP", icon: Users },
];

const RADIUS_NAV: NavItem[] = [
  { to: "/network/radius", label: "RADIUS/CoA", icon: Network },
];

const GPON_NAV: NavItem[] = [
  { to: "/network/gpon", label: "GPON/ONT", icon: RadioTower },
];

const ZABBIX_NAV: NavItem[] = [
  { to: "/monitoring/zabbix", label: "Zabbix alarms", icon: Shield },
];
...
const NOC_NAV: NavItem[] = [
  { to: "/noc/incidents", label: "NOC-инциденты", icon: Siren },
];
...
export function AppShell() {
  ...
  const navItems = (() => {
    ...
    if (hasMvpRole(role, ["support"])) {
      return [...ADMIN_NAV, ...SUBSCRIBER_OPS_NAV, ...RADIUS_NAV, ...GPON_NAV, ...ZABBIX_NAV, ...NOC_NAV];
    }
    if (hasMvpRole(role, ["billing"])) {
      return [...SUBSCRIBER_OPS_NAV, ...RADIUS_NAV];
    }
    if (hasMvpRole(role, ["noc_engineer"])) {
      return [...SUBSCRIBER_OPS_NAV, ...NETWORK_NAV, ...NOC_NAV];
    }
    return CUSTOMER_NAV;
  })();

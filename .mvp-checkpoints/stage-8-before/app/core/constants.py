...
class UserRole(str, Enum):
    ...
    def get_permissions(cls, role: "UserRole") -> Dict[str, bool]:
        ...
        if role == UserRole.BILLING:
            permissions["view_payments"] = True
            permissions["create_payment"] = True

        if role == UserRole.NOC_ENGINEER:
...
class GponOltStatus(str, Enum):
    ONLINE = "online"
    DEGRADED = "degraded"
    OFFLINE = "offline"


class GponOntStatus(str, Enum):
    ONLINE = "online"
    OFFLINE = "offline"
    BLOCKED = "blocked"
    ROGUE_SUSPECTED = "rogue_suspected"


class GponOntAction(str, Enum):
    REBOOT = "reboot"
    BLOCK = "block"
    UNBLOCK = "unblock"
    MARK_ROGUE_SUSPECTED = "mark_rogue_suspected"
    REFRESH_STATUS = "refresh_status"


class ZabbixAlarmType(str, Enum):
    BGP_DOWN = "bgp_down"
    VRRP_FAILOVER = "vrrp_failover"
    ERPS_RING_FAULT = "erps_ring_fault"
    OLT_OFFLINE = "olt_offline"
    LOW_OPTICAL_POWER = "low_optical_power"
    UPS_LOW_BATTERY = "ups_low_battery"
    DDOS_DETECTED = "ddos_detected"
    NAT_POOL_HIGH = "nat_pool_high"
...
class ZabbixAlarmStatus(str, Enum):
    ACTIVE = "active"
    ACKNOWLEDGED = "acknowledged"
    RESOLVED = "resolved"


class ZabbixSourceType(str, Enum):
...
class NocIncidentSeverity(str, Enum):
    LOW = "low"
    MEDIUM = "medium"
    HIGH = "high"
    CRITICAL = "critical"
...
class NocIncidentStatus(str, Enum):
    NEW = "new"
    ACKNOWLEDGED = "acknowledged"
...
class NocIncidentSource(str, Enum):
    MANUAL = "manual"
    ZABBIX = "zabbix"


class NocAffectedService(str, Enum):
    BGP = "bgp"
    VRRP = "vrrp"
    ERPS = "erps"
    GPON = "gpon"
...
MVP_ROLE_ALIASES: Dict[MvpRole, set[UserRole]] = {
    MvpRole.SUBSCRIBER: {UserRole.USER},
    MvpRole.SUPPORT: {UserRole.OPERATOR},
    MvpRole.BILLING: {UserRole.BILLING},
    MvpRole.NOC_ENGINEER: {UserRole.NOC_ENGINEER},
    MvpRole.ADMIN: {UserRole.ADMIN, UserRole.SUPER_ADMIN},
}
...
def normalize_user_role(role: UserRole | str | None) -> UserRole | None:
    if role is None:
        return None
    raw_role = role.value if hasattr(role, "value") else str(role)
    try:
...
def normalize_mvp_role(role: MvpRole | str | None) -> MvpRole | None:
    if role is None:
        return None
    raw_role = role.value if hasattr(role, "value") else str(role)
    try:
        return MvpRole(raw_role)
    except ValueError:
        return None


def mvp_roles_for_user_role(role: UserRole | str | None) -> set[MvpRole]:
    ...
    return {
        mvp_role
        for mvp_role, user_roles in MVP_ROLE_ALIASES.items()
        if user_role in user_roles
    }


def user_role_has_mvp_access(
    role: UserRole | str | None,
    allowed_roles: Iterable[MvpRole | str],
) -> bool:
    allowed = {
        normalized
        for normalized in (normalize_mvp_role(item) for item in allowed_roles)
    ...
    user_role = normalize_user_role(role)
    if user_role in MVP_ROLE_ALIASES[MvpRole.ADMIN]:
        return True

    return bool(mvp_roles_for_user_role(user_role) & allowed)

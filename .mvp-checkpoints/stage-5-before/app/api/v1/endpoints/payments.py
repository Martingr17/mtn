...
async def create_payment(
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Ошибка платёжного шлюза: {exc!s}") from exc
...
async def download_payment_statement_pdf_compat(
    year: Optional[int] = Query(None, ge=2020, le=2100),
    month: Optional[int] = Query(None, ge=1, le=12),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Return a PDF statement even when the period has no operations."""
    ...
    period_label = f"{month:02d}.{year}" if year and month else "all time"
    lines = [
        f"Subscriber: {current_user.phone}",
        f"Billing account: {current_user.billing_id}",
    ...
    if payments:
        for payment in reversed(payments):
            ...
            lines.append(f"{created_at} | {amount:.2f} RUB | {method} | {status_value} | ID {payment.id}")
    else:
        lines.append("No operations for the selected period yet.")

    lines.extend(["", f"Total top ups: {total_in:.2f} RUB"])
    pdf_bytes = build_statement_pdf("MTN payment statement", lines)
...
async def get_payment_methods_no_slash(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Compatibility route for payment methods without a trailing slash."""
    return await get_payment_methods(db=db, current_user=current_user)

...
function SubscriberDetailPage() {
  ...
  const canReadZabbix = hasMvpRole(role, ["support", "noc_engineer", "admin"]);
  ...
  const lowOpticalAlarmQuery = useQuery({
    queryKey: ["zabbix", "low-optical-power", subscriberId],
    queryFn: () =>
    ...
    enabled: Boolean(subscriberId) && canReadZabbix,
    retry: false,
  });
  ...
  const lowOpticalAlarm = (lowOpticalAlarmQuery.data?.items ?? []).find(
    (alarm) => alarm.source_id === gponOnt?.id,
  );
                ...
                {lowOpticalAlarm ? (
                  <div className="summary-row">
                    ...
                    <div className="stack-sm align-end-grid">
                      <StatusBadge tone="warning">Low optical power</StatusBadge>
                      <span className="muted">{lowOpticalAlarm.title}</span>
                    </div>
                  </div>
                ) : null}
                <div className="summary-row">

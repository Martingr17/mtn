...
import { AlertTriangle, CheckCircle2, RefreshCw, Send, ShieldAlert, Siren } from "lucide-react";
...
function telegramStatusTone(status: string): "success" | "warning" | "danger" | "neutral" {
  if (status === "sent") {
    return "success";
  }
  if (status === "failed") {
...
function ZabbixMonitoringPage() {
  ...
  const telegramLogsQuery = useQuery({
    queryKey: ["telegram-alerts", "zabbix-page"],
    queryFn: () => telegramAlertsService.list({ page_size: 10 }),
    enabled: canAction,
  });
  ...
  const createIncidentMutation = useMutation({
    mutationFn: (alarm: ZabbixAlarm) => incidentsService.createFromAlarm(alarm.id),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ["incidents"] });
      toast.success(
        result.result === "existing"
          ? `Incident #${result.incident.id} already linked.`
          : `Incident #${result.incident.id} created.`,
      );
    },
    onError: () => {
      toast.error("Failed to create incident from alarm.");
    },
  });
  const telegramMutation = useMutation({
    mutationFn: (alarm: ZabbixAlarm) => telegramAlertsService.sendZabbix(alarm.id),
    onSuccess: async (result) => {
    ...
    <div className="stack-lg">
                        ...
                        <div className="inline-actions">
                          ...
                          {canAction && alarm.status === "active" ? (
                            <Button
                              ...
                              onClick={() => createIncidentMutation.mutate(alarm)}
                            >
                              <Siren size={14} />
                              Create incident
                            </Button>
                          ) : null}
                          {canAction && alarm.status === "active" && alarm.severity === "critical" ? (
                            <Button
                              size="sm"
                              variant="secondary"
                              isLoading={telegramMutation.isPending}
                              onClick={() => telegramMutation.mutate(alarm)}
                            >
                              <Send size={14} />
                              Telegram
                            </Button>
      ...
      {canAction ? (
        <AnimatedReveal delay={0.18}>
          <Card className="table-shell stack-md">
            <div className="toolbar-row">
              <strong>Telegram alert log</strong>
              <span className="muted">{telegramLogsQuery.data?.total ?? 0} delivery facts</span>
            </div>
            {(telegramLogsQuery.data?.items ?? []).length ? (
              <div className="table-scroll">
                <table>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {(telegramLogsQuery.data?.items ?? []).map((item) => (
                      <tr key={item.id}>
                        <td>{item.entity_type} #{item.entity_id}</td>
                        <td>{item.title}</td>
                        <td>
                          <StatusBadge tone={telegramStatusTone(item.status)}>{item.status}</StatusBadge>
                        </td>
              ...
              <EmptyState
                icon={<Send size={20} />}
                title="No Telegram alerts yet"
                description="Critical Zabbix/NOC alerts will appear here after mock or real delivery attempts."
              />
            )}
          </Card>
        </AnimatedReveal>
